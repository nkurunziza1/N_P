...
const Subscription = () => {
    ...
    const col = ['id', 'client', 'Plate number && SimCard Number', 'createdAt', 'expiredAt'];
    const header = ['id', 'client', 'PlateNumber & Simcard Number', 'createdAt', 'expiredAt'];
    function handleDownloadExcel() {
        downloadExcel({
            ...
            tablePayload: {
                header: header,
                //@ts-ignore
                body: data.map((subscription) => ({
                    id: subscription.id,
                    client: subscription.vehicle?.client.firstName + ' ' + subscription.vehicle?.client.lastName,
                    'Plate number && SimCard Number': subscription.gps ? subscription.gps.serialNumber + ' - ' + subscription.gps.simcardNumber : '',
    ...
    const exportTable = (type: any) => {
        let columns: any = col;
        let records = UserData?.map((subscription) => ({
            id: subscription.id,
            createdAt: subscription.createdAt,
            expiredAt: subscription.expiredAt,
            client: subscription.vehicle?.client.firstName + ' ' + subscription.vehicle?.client.lastName,
        ...
        let filename = 'Subscription Data';
